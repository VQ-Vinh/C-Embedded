/*
You are implementing UART data transmission logic. 
A control register configures parity settings for data framing. 
...
uint8_t create_uart_frame(uint8_t data, UART_Control *ctrl) {
    if (ctrl->parity_enable == 0)  return data;                 // parrity disable
         
    uint8_t frame = data;
    uint8_t cout_bit_ones = 0;
    
    for (uint8_t i = 0; i < 7; i++) {
        if ((data >> i) & 1) cout_bit_ones++;                   // đếm số bit 1
    }

    if (ctrl->parity_type == 0) {                               // even parity
        if (cout_bit_ones % 2 != 0) frame = data | (1 << 7);    // nếu số bit 1 đang lẻ -> bật parity bit
    } else {                                                    // odd parity
        if (cout_bit_ones % 2 == 0) frame = data | (1 << 7);    // nếu số bit 1 đang chẳn -> bật parity bit
    }
